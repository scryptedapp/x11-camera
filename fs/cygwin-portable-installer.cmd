...
:: Sourced from https://github.com/vegardit/cygwin-portable-installer
...
"%CYGWIN_ROOT%\%CYGWIN_SETUP_EXE%" --no-admin ^
  ...
  --root "%CYGWIN_ROOT%" ^
  --local-package-dir "%CYGWIN_ROOT%\.pc" ^
  --no-shortcuts ^
  --no-desktop ^
  --delete-orphans ^
  --upgrade-also ^
...
(
  ...
  echo mv -f /etc/setup/setup.rc.mod /etc/setup/setup.rc ^|^| true
  echo.
  echo #
